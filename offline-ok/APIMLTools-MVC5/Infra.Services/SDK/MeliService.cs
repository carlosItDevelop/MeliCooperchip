...
{
	/// <summary>
	///  Esta classe não está em uso ainda nesta aplicação!!!
	/// </summary>
	public class MeliService
	{
		...
		private MeliService ()
		{
			service = new Meli(8883382550870001, "OVYpah5EkmFfqyeo7CyP1o9cZQWh1041");
		}
		...
		//public List<TotalDeVisitasNoSiteML> GetTotalVisitas(string id_user) {
		//    IRestResponse response = m.Get("/users/"+id_user+"/items_visits", ps);
		//    return = JsonConvert.DeserializeObject<TotalDeVisitasNoSiteML>(response.Content);
		//}
		...
		public ListaDeSubCategoriaComQtde GetSubCategoria(string CatId) {
			string resource = "/categories/" + CatId;
			IRestResponse response = service.Get(resource);
			//return JsonConvert.DeserializeObject<ListaDeSubCategoriaComQtde>(results.Content);
			return JsonConvert.DeserializeObject<ListaDeSubCategoriaComQtde>(response.Content);
		}

		public List<Categorias> GetCatergories(string site) {
			string resource = "sites/" + site + "/categories";
			IRestResponse results = service.Get(resource);
			return JsonConvert.DeserializeObject<List<Categorias>>(results.Content);
		}

		public List<TipoPublicidade> GetTipoPublicidade() {
            string resource = "/sites/MLB/listing_types";
			IRestResponse results = service.Get(resource);
			return JsonConvert.DeserializeObject<List<TipoPublicidade>>(results.Content);
		}
		...
		public List<Product> Search(string toSearch, string site ="MLB", string cat = "")
		{
		...
		{
			return service.GetAuthUrl("http://localhost:2724/Autenticacao/Index");
		}

		public void Authenticate (string code)
		{
			string url = "http://localhost:2724/Autenticacao/Index";
